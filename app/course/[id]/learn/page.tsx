										...
										<div className="border-t border-gray-200">
											{course.modules.filter(m => m.type !== 'ebook').map((module) => (
												<button
													...
													className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-all duration-200 ${currentModule?.module_id === module.module_id ? 'bg-purple-50 border-l-4 border-l-[#661FFF]' : 'hover:bg-gray-50 border-l-4 border-l-transparent'}`}
												>
													<div className="flex items-center gap-3 flex-1 min-w-0">
														...
														<div className="flex flex-col flex-1 min-w-0">
															<span className={`text-sm font-medium truncate ${currentModule?.module_id === module.module_id ? 'text-[#661FFF]' : 'text-gray-800'
																}`}>{module.title}</span>
															<span className="text-xs text-gray-500">
																{module.type === 'quiz' ? <span className="text-xs text-amber-600 font-medium">Kuis</span> : (module.duration > 0 ? formatDuration(module.duration) : 'Video')}
															</span>
														</div>
													</div>
												</button>
