...
// Interface untuk data kategori dan mentor dari API
interface Category {
    id: string;
    name: string;
    icon: string;
...
export default function BuatKelas() {
    ...
    const handleSaveClass = async () => {
        ...
        try {
            ...
            // Get selected mentor and category data
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                            <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        {categories.length === 0 && (
                                            <p className="text-xs text-orange-500 mt-1">
                                                Belum ada kategori. <a href="/admin/categories" className="underline">Tambah kategori</a>
                                            </p>
                                        )}
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    {mentors.length === 0 && (
                                        <p className="text-xs text-orange-500 mt-1">
                                            Belum ada mentor. <a href="/admin/mentors" className="underline">Tambah mentor</a>
                                        </p>
                                    )}
                                    {/* Preview mentor yang dipilih */}
                                    {mentorId && getSelectedMentor() && (
                                        <div className="mt-3 p-4 bg-purple-50 rounded-xl flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-purple-400 to-purple-600">
                                                <img
                                                    src={getSelectedMentor()?.photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(getSelectedMentor()?.name || '')}&background=6B21FF&color=fff`}
                                                    alt={getSelectedMentor()?.name}
                                                    className="w-full h-full object-cover"
                                                />
                                            </div>
                                            <div>
                                                <p className="font-semibold text-gray-900">{getSelectedMentor()?.name}</p>
                                                <p className="text-sm text-purple-600">{getSelectedMentor()?.title || 'Tidak ada jabatan'}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
